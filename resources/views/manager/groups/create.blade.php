    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100 p-6 sm:p-8 space-y-6">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900">Novo Grupo de Colaboradores</h3>
                    <p class="text-xs text-gray-500 mt-1">Crie um grupo de trabalho para facilitar a atribuição de trilhas de conhecimento a vários colaboradores de uma vez só.</p>
                </div>

                <form action="{{ route('manager.groups.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <!-- Nome -->
                    <div>
                        <label for="name" class="block text-sm font-semibold text-gray-700 mb-1">Nome do Grupo</label>
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Ex: Equipe Financeira" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition shadow-sm text-sm">
                        @error('name')
                    ...
                    <div class="border-t border-gray-100 pt-6">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Selecionar Colaboradores do Grupo</label>
                        <p class="text-xs text-gray-400 mb-4">Escolha quais colaboradores farão parte deste grupo inicialmente.</p>
                        
                        @if($users->isEmpty())
                            <p class="text-xs text-amber-600 font-semibold">⚠️ Não há colaboradores cadastrados nesta entidade para adicionar.</p>
                        @else
                                ...
                                @foreach($users as $user)
                                    <div class="flex items-start">
                                        <div class="flex items-center h-5">
                                            <input id="user_{{ $user->id }}" name="users[]" type="checkbox" value="{{ $user->id }}" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded transition">
                                        </div>
