    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('manager.groups.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none transition ease-in-out duration-150 shadow-md">
                    Novo Grupo
                </a>
            </div>
        </div>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100 p-6">
                <h3 class="text-lg font-bold text-gray-900 mb-6">Grupos da Entidade</h3>

                @if($groups->isEmpty())
                    <p class="text-sm text-gray-500 py-8 text-center">Nenhum grupo cadastrado.</p>
                @else
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nome do Grupo</th>
                                ...
                                @foreach($groups as $group)
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-500 font-medium">
                                            {{ $group->users_count }} colaboradores
                                        </td>

                                        <!-- Ações -->
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-bold space-x-2">
                                            <a href="{{ route('manager.groups.edit', $group) }}" class="text-indigo-600 hover:text-indigo-900 transition">
                                                Editar & Membros
                                            </a>
                                            <form action="{{ route('manager.groups.destroy', $group) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir este grupo?')">
                                                @csrf
                                                @method('DELETE')
