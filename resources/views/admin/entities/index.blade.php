    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.entities.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none transition ease-in-out duration-150 shadow-md">
                    Nova Entidade
                </a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100 p-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    <h3 class="text-lg font-bold text-gray-900">Entidades Cadastradas</h3>
                    
                    <form action="{{ route('admin.entities.index') }}" method="GET" class="flex items-center w-full md:max-w-md gap-2">
                        <input type="text" name="search" value="{{ $search }}" placeholder="Pesquisar por nome..." class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition shadow-sm text-sm">
                        ...
                        @if($search)
                            <a href="{{ route('admin.entities.index') }}" class="text-xs text-rose-600 hover:underline font-bold whitespace-nowrap px-1">
                                Limpar
                            </a>
                        @endif
                    </form>
                ...
                @if($entities->isEmpty())
                    <p class="text-sm text-gray-500 py-8 text-center">Nenhuma entidade cadastrada no sistema.</p>
                @else
                                ...
                                @foreach($entities as $entity)
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-bold space-x-2">
                                            <a href="{{ route('admin.entities.edit', $entity) }}" class="text-indigo-600 hover:text-indigo-900 transition">
                                                Editar
                                            </a>
                                            <form action="{{ route('admin.entities.destroy', $entity) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir esta entidade? Esta ação desvinculará todos os usuários e cursos a ela associados.')">
                                                @csrf
                                                @method('DELETE')
